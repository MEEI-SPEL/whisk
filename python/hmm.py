...
class EmmissionDistributions(object):
  ...
  """
    ...
    def _all_features(self, wvd, traj):
        data = zeros(
            (self._count_whiskers(wvd), len(self._features) + 3))  # the extra 3 cols are for class id, fid, wid
            ...
            for dst, logp in enumerate(row):
                for time in range(lrstates._nsteps + 1):
                    if src == 0:  # junk
                        ...
                        self._transitions.setdefault(map_state(src, time), {})[map_state(dst, time + 1)] = log2(
                            0.5)  # logp
...
def apply_model(wvd, model):
    ...
    statemap = dict(
        [('whisker%d' % i, i) for i in range(model._statemodel._nsteps)])  # TODO: there's got to be a better way
    traj = {}
    wrowcmp = lambda a, b: wcmp(a[1], b[1])
    for fid, wv in wvd.items():
        # print fid
