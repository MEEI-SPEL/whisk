...
def make_trajectories(wvd, datadict, side, n=None):
    ...
    for fid, v in datadict.items():
        good = [wid for wid, datarow in v.items() if
                datarow[0] == 1]  # filter for whiskers in frame that are in the good class
...
def plot_summary_data(wvd, traj, data):
    ...
    ax = subplot(211)
    vmin1, vmax1 = -90, 90
    ax.broken_barh([(i - 0.5, 1) for i, e in enumerate(mask) if not e], (vmin1, vmax1 - vmin1),
                   edgecolors=[(0, 0, 0, 0)], facecolors=[(0, 0, 0, 0.5)])
    xlabel('Time (frames)')
    ...
    ax.broken_barh([(i - 0.5, 1) for i, e in enumerate(mask) if not e], (vmin2, vmax2 - vmin2),
                   edgecolors=[(0, 0, 0, 0)], facecolors=[0, 0, 0, 0.5])
    xlabel('Time (frames)')
