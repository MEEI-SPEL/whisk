...
if __name__ == '__main__':
    ...
    parser.add_argument('-bg', type=int, default=63,
                        help="Do not show seeds in regions of the image darker than this threshold.")
    ...
    parser.add_argument('dest_', nargs='?', metavar='destination', default='render/%05d.png',
                        help="Specify where rendered images should be saved.")
    ...
    try:
        ...
        if N.maxframe < 0:
            N.maxframe = len(v) - 1
        ioff()
        play_seed_angles(v, N.maxr, N.thresh, alpha=0.75, bg=N.bg, every=slice(N.minframe, N.maxframe + 1),
                         renderto=N.dest_.strip())
    except:
