...
def draw_whisker(surf, w, radius=12, color=(0, 255, 255, 200), scale=1, color2=(0, 255, 255, 200), thick=0.0,
                 selected=False, mode=None, facehint='top'):  # DHO, changed default radius value.
    if len(w.x) == 0:
        return
...
def main(filename,
  ...
  """
    ...
    mode = {"auto": True, "tracing": False, "showcursorpos": show_cursor_pos, "showfps": show_fps, "draw": 0}
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 4:  # mouse wheel up
                    ...
                    if iframe in whiskers:
                        ...
                        if bestd < cursor_size:
                            ...
                            if current_whisker not in trajectories:
                    ...
                    if bestd < cursor_size:
                        ...
                        if fid in traj:
                            sid = traj[fid]
                            draw_whisker(screen, whiskers[max(im.tell() - 1, 0)][sid], color=(0, 0, 0),
                                         color2=(0, 0, 0), scale=scale[0], thick=1.0, mode=mode, facehint=facehint)
                    ...
                    if mode["auto"]:
                        e = {'pos': [int(scale[0] * ps[0]),
                             ...
                             'button': 1}  # left click
                        if (
                                    bestd < cursor_size) and im.tell() < N - 1:  # continue if nearby hit and not at
                            # end of movie
                    ...
                    if sd:
                        ...
                        if w:
                            ...
                            bg, a = render(screen, im, current_whisker, state, bg, scale[0], inc=inc, mode=mode,
                                           facehint=facehint)
                            if w:
                                draw_whisker(screen, w, color=(0, 0, 0), color2=(0, 0, 0), scale=scale[0], thick=1.0,
                                             mode=mode, facehint=facehint)
                            cursor_rect = pygame.draw.circle(screen, (0, 255, 0, 255), p, int(cursor_size * scale[0]),
                                                             1)
                ...
                if event.key == pygame.K_d:
                    ...
                    if event.mod & pygame.KMOD_SHIFT:
                        ...
                        h, m, s = trace.compute_seed_fields_windowed_on_objects(a, maxr=4, maxiter=1,
                                                                                window=(0.0, 0.0))
                    ...
                    if event.mod & pygame.KMOD_SHIFT:
                        ...
                        none_missing = lambda fid: reduce(And,
                                                          [fid in trajectories[t] and trajectories[t][
                                                              fid] in whiskers.get(fid, {}) for t in
                                                           [t for t in list(trajectories.keys()) if t >= 0]])
                    else:
                        none_missing = lambda fid: fid in trajectories.get(current_whisker, {}) and trajectories.get(
                            current_whisker, {}).get(fid, -1) in whiskers.get(fid, {})
...
def render(screen, im, current_whisker, state, bg, scale, **kwargs):
    ...
    if im.tell() in whiskers:
        ...
        if marked_seg in list(wv.keys()):
            ...
            draw_whisker(bg, wv[marked_seg], color=color, color2=coloralt, scale=scale, thick=2.5, selected=True,
                         mode={"draw": 0}, facehint=facehint)
