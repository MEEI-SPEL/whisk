...
class cMeasurements(Structure):
  ...
  """
    _fields_ = [("row", c_int),
                ...
                ("state", c_int),
                ("face_x", c_int),
                # // used in ordering whiskers on the face...roughly, the center of the face
                ("face_y", c_int),
                # //                                      ...does not need to be in image
                ("col_follicle_x", c_int),
                # // index of the column corresponding to the folicle x position
                ("col_follicle_y", c_int),
                # // index of the column corresponding to the folicle y position
...
class cDistributions(Structure):
  ...
  """
    _fields_ = [("n_measures", c_int),
                ("n_states", c_int),
                ("n_bins", c_int),
                ("bin_min", POINTER(c_double)),
                # // array of n_measures elements
                ("bin_delta", POINTER(c_double)),
                # // array of n_measures elements
                ("data", POINTER(
                    c_double))]  # // array of holding histogram information with dimensions (n_bins,n_measures,n_states)
...
class MeasurementsTable(object):
  ...
  """
    ...
    """
        ...
        wv = cWhisker_Seg.CastDictToArray(wvd)
    ...
    """
        mn, mx = c_int(), c_int()
        sorted = (self._sort_state is not None) and \
...
class Distributions(object):
    ...
    """
        ...
        self._vel = ctraj.Build_Velocity_Distributions(table._measurements, table._nrows,
                                                       nbins)  # changes table's sort order
...
def batch_make_measurements(sourcepath, ext='*.seq', label='curated'):
  ...
  """
    ...
    from python.ui.whiskerdata import load_trajectories
